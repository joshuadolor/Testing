    ...
    return (
        <nav className="sticky top-0 z-40 text-sm bg-white flex flex-col md:flex-row md:items-center md:justify-between p-4 py-2 gap-2 shadow-sm">
            <div className="flex justify-between items-center">
                {/* Logo */}
                <Link to="/" className="logo text-xl font-bold" onClick={toggleMenu}>
                    ShoeVista
                </Link>
            ...
            <div className={`flex flex-col md:flex-row md:items-center ${isOpen ? "block" : "hidden md:flex"}`}>
                ...
                <ul className="flex flex-col md:flex-row md:gap-4 md:pr-6">
                    {["/", "/about-us", "/shoes/men", "/shoes/women", "/shoes/kids"].map((path, index) => {
                        const labels = ["Home", "About Us", "Men", "Women", "Kids"];
                        return (
                            <li key={path} className="hover:bg-slate-100 transition-all duration-300 delay-100 p-2 rounded">
                                <NavLink to={path} onClick={toggleMenu} className={({ isActive }) => `${isActive ? "font-extrabold" : ""}`}>
                                    {labels[index]}
                                </NavLink>
                            </li>
                        );
                    ...
                    <div className="flex items-center gap-4">
                        <NavLink to="/wishlist" onClick={toggleMenu}>
                            <div className="relative">
                                <span className="w-4 text-center rounded-xl absolute bottom-4 left-4 text-xs bg-gray-900 text-white">{list.length > 0 ? list.length : ""}</span>
                                <img src="/Navbar/wishlist.png" alt="wishlist" className="w-6 h-6 hover:scale-110" />
                            </div>
                        </NavLink>
                        <NavLink to="/cart" onClick={toggleMenu}>
                            <div className="relative">
                                <span className="w-4 text-center rounded-xl absolute bottom-4 left-4 text-xs bg-gray-900 text-white">{cartList.length > 0 ? cartList.length : ""}</span>
